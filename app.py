# -*- coding: utf-8 -*-
"""
Бот-поисковик вакансий с многоуровневым обходом блокировок.
...
Переменные окружения:
  ...
  DEEPSEEK_API_KEY    — для диалогов (опционально)
...
async def fetch_hh_api(city_id: str, keyword: str, per_page: int = 15) -> List[Dict]:
...
async def fetch_superjob(city_name: str, keyword: str, count: int = 15) -> List[Dict]:
    if not SUPERJOB_KEY:
        return []
...
async def do_search(update: Update, context: ContextTypes.DEFAULT_TYPE, force: bool = False):
    ...
    try:
        ...
        for city_name, city_id in cities:
            for kw in keywords:
                try:
                    vacs = await fetch_hh_api(city_id, kw, per_page=10)
                ...
                if SUPERJOB_KEY:
                    try:
                        sj = await fetch_superjob(city_name, kw, count=10)
        ...
        if not good:
            good = scored[:MAX_VACANCIES_PER_CYCLE * 2]
        else:
            good = good[:MAX_VACANCIES_PER_CYCLE * 2]
